...
```{r}
cor.test(DelPrete.orig$`Anteroposterior diameter of the inlet`, DelPrete.orig$Femur.Head.Adj)
```

```{r}

model<-glm(DelPrete.orig$`Anteroposterior diameter of the inlet` ~ DelPrete.orig$Gender * DelPrete.orig$Location + DelPrete.orig$Femur.Head.Adj)
...
The ANCOVA test shows that there are statistical differences between the genders of the pelvic bones, the location of the pelvic bones, and the genders and locations combined.
...
?TukeyHSD
model.Tuk<-TukeyHSD(glm(DelPrete.orig$`Anteroposterior diameter of the inlet` ~ DelPrete.orig$Gender * DelPrete.orig$Location))
as.data.frame(tidy(model.Tuk)) %>%
    filter(adj.p.value < .05)
model.Tuk
```

```{r}
cor.test(DelPrete.orig$`Pelvic breadth`, DelPrete.orig$Femur.Head.Adj)
```

```{r}

model<-glm(DelPrete.orig$`Pelvic breadth` ~ DelPrete.orig$Gender * DelPrete.orig$Location + DelPrete.orig$Femur.Head.Adj)
...
library(reshape2)
library(broom)
model.Tuk<-TukeyHSD(glm(DelPrete.orig$`Pelvic breadth` ~ DelPrete.orig$Gender * DelPrete.orig$Location))
as.data.frame(tidy(model.Tuk)) %>%
    filter(adj.p.value < .05)
model.Tuk
```
...
results<-manova(cbind(DelPrete.orig$`Anteroposterior diameter of the inlet`,  DelPrete.orig$`Pelvic breadth`, DelPrete.orig$`Anterioposterior space of outlet adj`, DelPrete.orig$`Bi-spinous diameter`, DelPrete.orig$`Transverse diameter of the outlet`) ~ DelPrete.orig$Gender * DelPrete.orig$Location + DelPrete.orig$Femur.Head.Adj)
summary.aov(results)
...
results<-Manova(model, test.statistic="F"
               )
results
