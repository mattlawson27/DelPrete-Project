...
descrip.Del <- function(resp,pred){
  ...
  favstats.names <- list("min", "Q1", "median", "Q3", "max", "mean", "sd", "n")
  descrip.array <- array(dim = c(length(resp),length(favstats.names)), dimnames = list(names(resp),favstats.names))
  for (i in 1:length(resp)){
    ...
    gf_dhistogram(~ resp[[i]]) %>%
               gf_fitdistr(dist = "norm")
    
    hist(resp[[i]], xlab = names(resp[i]), freq = FALSE) %>%
               gf_fitdistr(dist = "norm")
...
cor.all.lin <- function(resp,pred) {
  ...
  for (k in 1:length(pred)){
    ...
    plot(resp[[i]], pred[[k]], xlab = names(resp[i]), ylab = names(pred[k]))
  }
  }
...
DelPrete.orig$`femur head diameter`[498]
mean(as.numeric(DelPrete.orig$`femur head diameter`))
DelPrete.orig$`femur head diameter`[498] <- mean(DelPrete.orig$`femur head diameter`)
...
Del.orig.pred.F <- list(Femur.Head.Adj = DelPrete.orig.F$Femur.Head.Adj)
```
...
gf_dhistogram(~ Del.orig.resp.F[[1]]) %>%
               gf_fitdistr(dist = "norm")
gf_dhistogram(~ Del.orig.resp.F[[2]]) %>%
               gf_fitdistr(dist = "norm")
gf_dhistogram(~ Del.orig.resp.F[[3]]) %>%
               gf_fitdistr(dist = "norm")
gf_dhistogram(~ Del.orig.resp.F[[4]]) %>%
               gf_fitdistr(dist = "norm")
gf_dhistogram(~ Del.orig.resp.F[[5]]) %>%
               gf_fitdistr(dist = "norm")

```

```{r}
cor(Del.orig.resp.F[[1]], Del.orig.pred.F[[1]])
plot(Del.orig.resp.F[[1]], Del.orig.pred.F[[1]])
```


```{r}
cor.all.lin(resp = Del.orig.resp.F, pred = Del.orig.pred.F)
```
